...
screen_height = (tile_size * 15) + margin

screen = pygame.display.set_mode((screen_width, screen_height))
pygame.display.set_caption('Edit_Level')
...
blob_img = pygame.image.load('img/qvat0.png')
...
coinbox_img=pygame.image.load('img/coinbox.png')
...
# Tạo world_data với 15 hàng và 20 cột
world_data = []
for row in range(15):
    r = [0] * 20
    world_data.append(r)
...
for tile in range(20):
    world_data[14][tile] = 2  # Hàng cuối cùng là cỏ
    world_data[0][tile] = 1   # Hàng đầu tiên là đất
for tile in range(15):
    world_data[tile][0] = 1   # Cột đầu tiên là đất
    world_data[tile][19] = 1  # Cột cuối cùng là đất
...
def draw_grid():
    for c in range(cols + 1):  # Số cột là 20
        # Vẽ các đường thẳng dọc (vertical lines)
        pygame.draw.line(screen, white, (c * tile_size, 0), (c * tile_size, screen_height - margin))
        
    for r in range(16):  # Số hàng là 15 (tăng thêm 1 để vẽ đủ hàng)
        # Vẽ các đường thẳng ngang (horizontal lines)
		...
		for col in range(20):
			if world_data[row][col] > 0:
				...
				if world_data[row][col]==9:
					#heart
					img=pygame.transform.scale(heart_img,(tile_size,tile_size))
					screen.blit(img,(col*tile_size,row*tile_size))
